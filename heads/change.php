...
$newPassword = isset($_POST['Npassword']) ? md5($_POST['Npassword']) : '';



// Reset the user's password
$sql = "UPDATE tblemployees SET Password = '$newPassword' WHERE EmailId = '$email'";
	...
	echo '<script>
          $(document).ready(function() {
            $("#btn").click(function(e) {
              e.preventDefault();
              var $state = $("#loginForm button > .state");
              $("#loginForm").addClass("loading");
              $state.html("Authenticating");
              setTimeout(function() {
                $("#loginForm").addClass("ok");
                $state.html("Reset Successfully!");
                setTimeout(function() {
                  $state.html("Reset");
                  $("#loginForm").removeClass("ok loading");
                }, 4000);
              }, 3000);
            });
          });
...
<!-- 
		...
		<div class="wrapper">
			<form class="login" id="loginForm" method="POST" onsubmit="return passvalid()">
				...
				<input type="password" name="Npassword" id="new_password" placeholder="Password"><br>
