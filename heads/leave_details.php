...
<?php
	...
	{ 
		...
		if ($status === '2') {
			$result = mysqli_query($conn,"update tblleaves, tblemployees set tblleaves.registra_remarks='$description',tblleaves.Status='$status',tblleaves.AdminRemarkDate='$admremarkdate', tblleaves.registra_remarks = '$reg_remarks', tblleaves.admin_status = '$reg_status' where tblleaves.empid = tblemployees.emp_id AND tblleaves.id='$did'");
		...
		elseif ($status === '1') {
				$result = mysqli_query($conn,"update tblleaves, tblemployees set tblleaves.registra_remarks='$description',tblleaves.Status='$status',tblleaves.AdminRemarkDate='$admremarkdate' where tblleaves.empid = tblemployees.emp_id AND tblleaves.id='$did'");

				if ($result) {
			     	echo "<script>alert('Leave updated Successfully');</script>";

					$con = mysqli_connect('localhost','root','','leave_staff');

					$qur = "SELECT empid FROM tblleaves WHERE id='$did' LIMIT 1";
					$res = mysqli_query($con,$qur);
					$row = mysqli_fetch_array($res);
					$empid = $row['empid'];

					mysqli_query($con,"UPDATE tblemployees SET Av_leave='$REMLEAVE' WHERE emp_id='$empid'");
					if ($levtyp == 'Casual Leave') {

						$qur = "SELECT Casual_leaves FROM tblemployees WHERE emp_id='$empid' LIMIT 1";
						$res = mysqli_query($con,$qur);
						$row = mysqli_fetch_array($res);
						$casLev = $row['Casual_leaves'];

						$casLev = $casLev - $num_days;

						mysqli_query($con,"UPDATE tblemployees SET Casual_leaves='$casLev' WHERE emp_id='$empid'");

					} else {

						$qur = "SELECT Medical_leaves FROM tblemployees WHERE emp_id='$empid' LIMIT 1";
						$res = mysqli_query($con,$qur);
						$row = mysqli_fetch_array($res);
						$medLev = $row['Medical_leaves'];

						$medLev = $medLev - $num_days;

						mysqli_query($con,"UPDATE tblemployees SET Medical_leaves='$medLev' WHERE emp_id='$empid'");
					}
					} else{
					  die(mysqli_error());
				   }
...
<?php 
	...
	$con = mysqli_connect('localhost','root','','leave_staff');
	$qur = "SELECT admin_status FROM tblleaves WHERE id='$did' LIMIT 1";
	$res = mysqli_query($con,$qur);
	$row = mysqli_fetch_array($res);
	$adiminapro = $row['admin_status'];
	if ($adiminapro == 1) {
		$actiondisply = '';
	} else {
		$actiondisply = 'display:none;';
	}
						...
						<div class="row">
							...
							<div class="col-md-4 col-sm-12">
								<div class="form-group">
									...
									<input type="text" name="levtyp" class="selectpicker form-control" data-style="btn-outline-info" readonly value="<?php echo htmlentities($result->LeaveType);?>">
								</div>
							</div>
							<div class="col-md-4 col-sm-12">
								<div class="form-group">
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<div class="modal-footer justify-content-center">
										<button class="btn btn-primary" id="action_take" data-toggle="modal" data-target="#success-modal" style="<?php echo $actiondisply ?>" >Take&nbsp;Action</button>
									</div>
								</div>
							</div>
